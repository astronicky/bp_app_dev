/*
 * Created by Justice on Sat Jun 12 2021
 *
...
export class CommentNewComponent extends Component {
    ...
    tapGifIcon = () => {
        ...
        this.props.navigation.navigate('ProfileGifSearchComponent', { postId: post.id })
    }
    ...
    sendComment = () => {
        ...
        if (isLoading != true && post != undefined) {
            // create a text post
            if (preview != undefined) {
                this.props.doGivePostLinkComment(ownerId, postId, comment, preview, urls, pageLocation)
            } else if (media.length != 0) {
                this.props.doGivePostMediaComment(ownerId, postId, comment, media, pageLocation)
            } else {
                if (_.isEmpty(comment) == false) {
                    this.props.doGivePostTextComment(ownerId, postId, comment, pageLocation)
                }
            }
        }
...
const mapDispatchToProps = (dispatch) => {
    const _action = {
        dispatch,
        doGivePostTextComment: (ownerId, postId, comment, pageLocation) => dispatch(actGivePostTextComment(ownerId, postId, comment, pageLocation)),
        doGivePostLinkComment: (ownerId, postId, comment, preview, urls, pageLocation) => dispatch(actGivePostLinkComment(ownerId, postId, comment, preview, urls, pageLocation)),
        doGivePostMediaComment: (ownerId, postId, comment, images, pageLocation) => dispatch(actGivePostMediaComment(ownerId, postId, comment, images, pageLocation)),
